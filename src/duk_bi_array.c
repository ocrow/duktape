...
DUK_LOCAL duk_harray *duk__arraypart_fastpath_this(duk_context *ctx) {
	...
	if (!DUK_TVAL_IS_OBJECT(tv)) {
		DUK_D(DUK_DPRINT("reject array fast path: not an object"));
		return NULL;
	}
	...
	if ((flags_value & flags_mask) != flags_bits) {
		DUK_D(DUK_DPRINT("reject array fast path: object flag check failed"));
		return NULL;
	}
	...
	if (((duk_harray *) h)->length > DUK_HOBJECT_GET_ASIZE(h)) {
		DUK_D(DUK_DPRINT("reject array fast path: length > array part size"));
		return NULL;
	}

	DUK_D(DUK_DPRINT("array fast path allowed for: %!O", (duk_heaphdr *) h));
	return (duk_harray *) h;
}
...
DUK_LOCAL duk_ret_t duk__array_pop_fastpath(duk_context *ctx, duk_harray *h_arr) {
	...
	DUK_ASSERT(h_arr->length <= DUK_HOBJECT_GET_ASIZE((duk_hobject *) h_arr));
...
DUK_LOCAL duk_bool_t duk__array_push_fastpath(duk_context *ctx, duk_harray *h_arr) {
	...
	len = h_arr->length;
	tv_arraypart = DUK_HOBJECT_A_GET_BASE(thr->heap, (duk_hobject *) h_arr);
	DUK_ASSERT(DUK_HOBJECT_GET_ASIZE((duk_hobject *) h_arr) == 0 || tv_arraypart != NULL);
	...
	if (len + n > DUK_HOBJECT_GET_ASIZE((duk_hobject *) h_arr)) {
		/* XXX: extend support, rework hobject code a little bit. */
		return 0;
	}
...
DUK_INTERNAL duk_ret_t duk_bi_array_prototype_push(duk_context *ctx) {
	...
	/* XXX: fast path will need some kind of 'guarantee space' operation
